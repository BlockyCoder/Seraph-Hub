...
fadeIn(exampleElement, duration)

fadeOut(exampleElement, duration)

slide(exampleElement, duration)

scale(exampleElement, duration)

rotate(exampleElement, duration)

pulse(exampleElement, duration)

shake(exampleElement, duration)

colorChange(exampleElement, Color3.new(1, 0, 0), duration)

bounce(exampleElement, duration)

flip(exampleElement, duration)

wobble(exampleElement, duration)

fadeAndMove(exampleElement, duration)

rotateAndScale(exampleElement, duration)

flash(exampleElement, duration)

expand(exampleElement, duration)

contract(exampleElement, duration)
...
function Library:Create(xHubName)
	...
	if  plr.PlayerGui:FindFirstChild(xHubName) then
		plr.PlayerGui[xHubName]:Destroy()
	end
	...
	ImageButtonK.MouseButton1Click:Connect(function()
		...
		if Menu.Visible then
			...
			--scale(Menu, 0.3)  -- Scale down Main frame
			...
			--scale(Menu, 0.3)  -- Scale up Main frame
			--rotate(Menu, 0.3)  -- Rotate Main frame
			--pulse(Menu, 0.3)  -- Pulse Main frame
			--shake(Menu, 0.3)  -- Shake Main frame
			--colorChange(Menu, 0.3)  -- Change color of Main frame
	...
	function Library:Tab(TabNam, IconId)
		TabNam = TabNam or "Tab"
		...
		TextLabel.Position = UDim2.new(IconId and 0.2 or 0.094, 0, 0, 0)
		...
		ExampleSection.BackgroundTransparency = 1
		...
		SectionLabel.Size = UDim2.new(0, 255, 0, 34)
		SectionLabel.Font = Enum.Font.Gotham
		SectionLabel.Text = TabNam
		...
		Example.MouseButton1Click:Connect(function()
			...
			Example.BackgroundColor3 = Color3.fromRGB(17, 146, 172)
		end)
	...
	function Library:TextBox(parent, titleText, defaultValue, callback)
		...
		TextBox.Active = true  -- Make it interactive
		...
		-- Apply UIStroke to TextBox and Status
		local UIStroke = Instance.new("UIStroke")
		...
		function Library:settext(newText)
		...
		function Library:Toggle(tab, toggleName, callback)
			...
			local function updateVisualState()
				TweenService:Create(Icon, TweenInfo.new(0.15), {ImageTransparency = isToggled and 0 or 1}):Play()
			...
			Toggle.MouseEnter:Connect(function()
				Tsound:Play()
				TweenService:Create(uistrokeT, TweenInfo.new(0.1), {Color = Color3.fromRGB(86, 34, 34)}):Play()
			end)
	...
	function Library:Keybind(tab, keybindName, defaultKey, callback)
		...
		Keybind.Position = UDim2.new(0, 0, 0.439, 0)  
		...
		UIStroke:Clone().Parent = Status
		UIStroke:Clone().Parent = Keybind
		...
		TextLabel_2.Text = tostring(defaultKey):gsub("Enum.KeyCode.", "")  
			...
			for i, option in ipairs(options) do
				...
				if optionButton then
					...
					if table.find(selectedOptions, option) then
						Checkmark.Visible = true
						optionButton.Checkbox.BackgroundColor3 = Color3.fromRGB(0, 85, 127)
					else
						Checkmark.Visible = false
						optionButton.Checkbox.BackgroundColor3 = Color3.fromRGB(30, 30, 30)
					end
				end
			end
	...
	function Library:Dropdown(tab, dropdownName, options, callback, settings)
		...
		local isOpen = false
		local openDropdown = nil  -- Track the currently open dropdown
		...
		List.ClipsDescendants = true
		List.CanvasSize = UDim2.new(0, 0, 0, #options * (25 + 3))
		List.ScrollBarThickness = 6
		List.ScrollingDirection = Enum.ScrollingDirection.Y
		List.Visible = false
		...
		local function updateOptions(filterText)
			for i, optionText in ipairs(options) do
				local optionButton = List:FindFirstChild("Option" .. i)
				if not filterText or optionText:lower():find(filterText:lower()) then
					optionButton.Visible = true
				else
					optionButton.Visible = false
				end
			end
			List.CanvasSize = UDim2.new(0, 0, 0, #List:GetChildren() * (25 + 3))
		end
		...
		for i, optionText in ipairs(options) do
			...
			Dot.Name = "Dot"
			Dot.Parent = Option
			Dot.BackgroundColor3 = Color3.fromRGB(6, 53, 62)
			Dot.BorderSizePixel = 0
			Dot.Size = UDim2.new(0, 15, 0, 15)
			Dot.Position = UDim2.new(0, 5, 0.5, -7.5)
			...
			if showIcons then
				...
				OptionIcon.Parent = Option
				OptionIcon.BackgroundTransparency = 1.000
				OptionIcon.Size = UDim2.new(0, 20, 0, 20)
				OptionIcon.Position = UDim2.new(0, 30, 0.5, -10)
				OptionIcon.Image = "rbxassetid://6031091000"
			end
		...
		function Library:UpdateDropdown(dropdown, newOptions)
			assert(type(newOptions) == "table" and #newOptions > 0, "Options should be a non-empty table.")

			local List = dropdown.Parent:FindFirstChild("List")
			...
			for i, optionText in ipairs(newOptions) do
				...
				Option.MouseButton1Click:Connect(function()
					dropdown.Main.Title.Text = optionText
					List.Visible = false
			...
			List.CanvasSize = UDim2.new(0, 0, 0, #newOptions * (25 + 3))
		end

		function Library:Setdrop(optionText)
			for _, option in ipairs(List:GetChildren()) do
				if option:IsA("TextButton") and option.Text == optionText then
					...
					self.Main.Title.Text = optionText
